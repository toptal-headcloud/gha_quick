...
const gitRevisionPlugin = (() => {
  ...
  const runCommand = (command = ':') => `log -1 &> /dev/null || ${command}`;

  const isForceRefresh = process.env.ISFORCEREFRESH?.toLowerCase() === 'true';
  const versionPostfix = isForceRefresh ? ':force-refresh' : '';

  return fs.existsSync('.git')
    ? // allow non-annotated tags, which we use
      new GitRevisionPlugin({
        versionCommand: `describe --always --tags | tr -d '\\n' && echo '${versionPostfix}'`,
      })
    : new GitRevisionPlugin({
        commithashCommand: runCommand(`echo '${process.env.COMMITHASH}'`),
        versionCommand: runCommand(
          `echo '${process.env.VERSION}${versionPostfix}'`
        ),
